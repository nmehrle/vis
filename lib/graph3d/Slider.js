...
function Slider(container, options) {
  ...
  if (this.visible) {
    ...
    this.frame.play.style.width = '45px';
...
// Extend Slider with an Emitter mixin
...
Slider.prototype.playNext = function() {
  ...
  if (index < this.values.length - 1) {
    ...
    if (!this.playLoop && index == this.values.length-1) {
...
Slider.prototype.togglePlay = function() {
  if(this.getIndex() == this.values.length-1) {
    this.reset();
  }
...
Slider.prototype.play = function() {
  ...
  this.playTimeout = 1; //define playTimeout so we can check if playing
  this.playNext();
};
...
/**
 * Reset to begining and start playing
 */
...
Slider.prototype.setIndex = function(index) {
  if (index < this.values.length) {
    this.emit("sliderSetIndex",index);
    this.index = index;

    if(this.frame) {
      if (index == this.values.length-1) {
        this.frame.play.value = 'Reset';
      }
