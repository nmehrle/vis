...
var Delaunay = require('../module/delaunay.js')
...
Graph3d.prototype._checkValueField = function (data) {
  ...
  var hasValueField = this.style === Graph3d.STYLE.BARCOLOR
                   ...
                   || this.style === Graph3d.STYLE.DOTIMG;
...
/**
 * Checks if sourse is defined and is valid
 * For dot-img style
 */
Graph3d.prototype._checkSourceField = function (data) {
  if(this.style != Graph3d.STYLE.DOTIMG) return;

  if (this.colSource === undefined) {
    throw new Error('Expected data to have field source for graph style dot-img');
  }

  if (data[0][this.colSource] === undefined) {
    throw new Error('Expected data to have field source for graph style dot-img');
  }
};
...
Graph3d.prototype._dataInitialize = function (rawData, style) {
  ...
  if (data[0].hasOwnProperty('source')) {
    this.colSource = 'source';
  }
...
Graph3d.prototype._getDataPoints = function (data) {
  ...
  if (this.style === Graph3d.STYLE.GRID ||
  ...
  else if (this.style === Graph3d.STYLE.TRISURF) {
    var vertices = [];
    for (i = 0; i < this.getNumberOfRows(data); i++) {
      point = new Point3d();
    ...
    for (i = 0; i<triangulation.length; i+=3) {
      var basePoint = dataPoints[triangulation[i]];
      if(basePoint.triangles === undefined) {
        basePoint.triangles = [];
      }
      basePoint.triangles.push([dataPoints[triangulation[i+1]],
                               dataPoints[triangulation[i+2]]]);
    }
  }
    ...
    for (i = 0; i < data.length; i++) {
      ...
      else if (this.style === Graph3d.STYLE.DOTIMG) {
        this._checkSourceField(data);
        point.source = data[i][this.colSource];
      }
 ...
 */
Graph3d.prototype.animationSetFrame = function(frame) {
  if (!this.frame.filter || !this.frame.filter.slider)
    throw new Error('No animation available');

  this.frame.filter.slider.setIndex(frame);
};
...
Graph3d.prototype.setOptions = function (options) {
  ...
  this._checkColorScale(this.colorScale);
...
Graph3d.prototype.setPointDrawingMethod = function() {
  ...
  switch (this.style) {
    ...
    case Graph3d.STYLE.DOTSIZE:
      ...
      break;
    case Graph3d.STYLE.DOTIMG:
      method = Graph3d.prototype._redrawDotImgGraphPoint;
      break;
    case Graph3d.STYLE.SURFACE:
      method = Graph3d.prototype._redrawSurfaceGraphPoint;
      break;
    case Graph3d.STYLE.TRISURF:
      method = Graph3d.prototype._redrawTriSurfGraphPoint;
      break;
...
Graph3d.prototype._redrawLegend = function() {
  ...
  if (isSizeLegend === false) {
    ...
    for (y = ymin; y < ymax; y++) {
      ...
      if(this.colorScale) {
        color = this._colorFromScale(f);
      }
      else {
        color = this._hsv2rgb(f*240, 1, 1);
      }
...
/**
 * Passes events from slider class forward
 */
Graph3d.prototype._passFilterEvents = function() {
  var me = this;

  this.frame.filter.slider.on('sliderSetIndex', function(e) {
    me.emit('animationSetFrame',e);
  })
}
...
Graph3d.prototype._redrawFilter = function() {
  ...
  if (this.dataFilter) {
    ...
    slider.setPlayLoop(this.animationLoop);
    this._passFilterEvents();
...
/** 
 * Checks if a given colorScale is valid
 */
Graph3d.prototype._checkColorScale = function(colorScale) {
  //undefined colorScale is valid
  if(colorScale === undefined) {
    return;
  }

  var badColorScaleError = new Error('ColorScale must be an array of valid colors of at least length 2');
  //If colorScale is defined it must be an array 
  //It must be length 2 or more
  if(!(colorScale.constructor === Array) || colorScale.length < 2) {
    throw badColorScaleError;
  };

  var formatedScale = [];
  for (var i = 0; i < colorScale.length; i++) {
    //check for valid hex color and if found convert to rgb
    if(/(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(colorScale[i])) {
      var shorthandRegex = /^#?([a-f\d])([a-f\d])([a-f\d])$/i;
      var hex = colorScale[i].replace(shorthandRegex, function(m, r, g, b) {
        return r + r + g + g + b + b;
      });
  ...
  this.colorScale = formatedScale;
};


/**
 * Returns an RGB color from the inputed color scale based on given length along colorSale
 * @param {Number} f  Fraction, value between 0 and 1, representing length along colorScale
 * Linearly interpolates colors in RGB space between set points on colorscale
 */
Graph3d.prototype._colorFromScale = function(f) {
  if (f<0) f=0;
  if (f>1) f=1;

  var colorPosition = f * (this.colorScale.length-1);

  var below = this.colorScale[Math.floor(colorPosition)];
  var above = this.colorScale[Math.ceil(colorPosition)];

  var frac  = colorPosition - Math.floor(colorPosition);  

  var r = below.r + frac * (above.r-below.r);
  var g = below.g + frac * (above.g-below.g);
  var b = below.b + frac * (above.b-below.b);
...
Graph3d.prototype._getColorsRegular = function(point) {
  ...
  var f = (1 - (point.point.z - this.zRange.min) * this.scale.z  / this.verticalRatio); 
  if(this.colorScale) {
    color = this._colorFromScale(f);
    borderColor = color;
  }
    ...
    color       = this._hsv2rgb(f*240, 1, 1);
    borderColor = this._hsv2rgb(f*240, 1, 0.8);
  }
...
Graph3d.prototype._getColorsColor = function(point) {
  ...
  if (typeof point.point.value === "string") {
    ...
    var f = (1 - (point.point.value - this.valueRange.min) * this.scale.value);
    if(this.colorScale) {
      color = this._colorFromScale(f);
      borderColor = color;
    }
...
/**
 * Draw single datapoint for graph style 'dot-img'.
 * Assumes point style field is an image that has already been loaded.
 */
Graph3d.prototype._redrawDotImgGraphPoint = function(ctx, point) {
  if (point.point.source instanceof HTMLImageElement) {
    var dotSize  = this._dotSize();
    var fraction = (point.point.value - this.valueRange.min) / this.valueRange.range(); 
    var size     = dotSize/2 + 2*dotSize * fraction;
    var colors   = this._getColorsSize();
    ...
    //draw border on image only if this.datacolor.strokeWdith > 0
    var lineWidth = this._getStrokeWidth(point);
    if (lineWidth > 0) {
    ...
    //fill image background if this.datacolor.fill != 'none'
    if (colors.fill != 'none' && colors.fill != 'NONE' && colors.fill != 'None') {
      ctx.fillStyle = colors.fill;
      ctx.fill();
    }

    var destX    = point.screen.x - radius;
    var destY    = point.screen.y - radius;
    var destSize = 2*radius;
    ctx.drawImage(point.point.source,destX,destY,destSize,destSize);
    ctx.restore();
  }
  else {
    throw new Error('Expected source to be instanceof HTMLImageElement'+
                    ' for graph style dot-img');
  }
}
...
Graph3d.prototype._redrawSurfaceGraphPoint = function(ctx, point) {
  ...
  if (topSideVisible) {
    ...
    var f    = (1 - (zAvg - this.zRange.min) * this.scale.z  / this.verticalRatio)
    

    if (this.showShadow) {
      if(this.colorScale) {
        fillStyle = this._colorFromScale(f);
      }
      else {
        var v = Math.min(1 + (crossproduct.x / len) / 2, 1);  // value. TODO: scale
        fillStyle = this._hsv2rgb(f * 240, 1, v);
      }
      ...
      if(this.colorScale) {
        ...
        fillStyle = this._hsv2rgb(f * 240, 1, 1);
      }
  ...
  lineWidth = this._getStrokeWidth(point);
  ctx.lineWidth = lineWidth;

  //when strokewidth is zero, draw polygon borders same color as polygon fill
  //simply not calling ctx.stroke() in polygon method left gaps between polygons
  if(lineWidth == 0) {
    strokeStyle = fillStyle;
  }
...
/**
 * Draw single datapoint for graph style 'tri-surf'.
 */
Graph3d.prototype._redrawTriSurfGraphPoint = function(ctx,point) {
  var fillStyle;
  var strokeStyle;
  var lineWidth;

  if (point.triangles === undefined) {
    return;
  }

  for (var i = 0; i < point.triangles.length; i++) {
    var pointB = point.triangles[i][0];
    var pointC = point.triangles[i][1];

    var zAvg = (point.point.z + pointB.point.z + pointC.point.z) / 3;
    var f    = (1 - (zAvg - this.zRange.min) * this.scale.z  / this.verticalRatio);
    ...
    var points = [point, pointB, pointC];
    this._polygon(ctx,points,fillStyle,strokeStyle);
  }
...
Graph3d.prototype._drawGridLine = function(ctx, from, to) {
  ...
  if(this.colorScale) {
      strokeColor = this._colorFromScale(f);
  }
  else {
    strokeColor   = this._hsv2rgb(f*240, 1, 1);
  }
...
Graph3d.prototype._redrawLineGraphPoint = function(ctx, point) {
  ...
  if(point.point.value != 'END_SEGMENT') {
