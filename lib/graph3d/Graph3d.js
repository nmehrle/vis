...
/**
 * Step animation into previous frame
 */
Graph3d.prototype.animationPrevFrame = function() {
  if (!this.frame.filter || !this.frame.filter.slider)
    throw new Error('No animation available');

  this.frame.filter.slider.prev();
};

/**
 * Step animation into next frame
 */
Graph3d.prototype.animationNextFrame = function() {
  if (!this.frame.filter || !this.frame.filter.slider)
    throw new Error('No animation available');

  this.frame.filter.slider.next();
};

/**
 * Toggle animation play/stop
 */
Graph3d.prototype.animationToggle = function() {
  if (!this.frame.filter || !this.frame.filter.slider)
    throw new Error('No animation available');

  this.frame.filter.slider.togglePlay();
};
...
Graph3d.prototype._passFilterEvents = function() {
  var me = this;
  this.frame.filter.slider.on('sliderPrevFrame',function() {
    me.emit('animationPrevFrame');
  });

  this.frame.filter.slider.on('sliderNextFrame',function() {
    me.emit('animationNextFrame');
  });

  this.frame.filter.slider.on('sliderTogglePlay',function() {
    me.emit('animationTogglePlay');
  });
}
